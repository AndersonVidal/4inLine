...
void enfileirar(int tamanho, char fila[tamanho], int item) {
...
int getPosicaoVazia(int tamanho, char fila[tamanho], int jogador) {
    int qtdJogador = 0;
    int qtdVazio = 0;
    int posicaoVazia = -1;
...
int procuraNaVertical(char **tabuleiro, int jogador, int tamanho) {
    char filaAtual[tamanho];
...
peças do jogador Y. Se for encontrada uma sequência, retorna a posição da casa vazia; 
caso contrário, retorna -1. */
int procurarNaHorizontal(char **tabuleiro, int jogador, int tamanho) {
    char filaAtual[tamanho];
    for (int lin = LINHAS - 1; lin >= 0; lin--) {
        memset(filaAtual, -1, sizeof filaAtual);
        for (int col = 0; col < COLUNAS; col++) {
...
caso contrário, retorna -1. */
int procurarNaDiagonalPrimaria(char **tabuleiro, int jogador, int tamanho) {
    char filaAtual[tamanho];
...
de N - 1 peças do jogador Y. Se for encontrada uma sequência, retorna a posição da casa vazia; 
caso contrário, retorna -1. */
int procurarNaDiagonalSecundaria(char **tabuleiro, int jogador, int tamanho) {
    char filaAtual[tamanho];
    for (int topo_linha = 2; topo_linha >= 0; topo_linha--) {
...
int getPosicao(char **tabuleiro, int jogador, int tamanho) {
    ...
    return possibilidades[rand() % ++counter] + 1   ;
}
